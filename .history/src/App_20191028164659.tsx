...
class ControlPanel extends React.Component<ControlPanelProps, ControlPanelState> {
  ...
  sendMessage() {
    if (this.props.sendMessage != undefined) {
      ...
      //store.dispatch({type:"sendMessage"})
    }
  }
...
const mapDispatchToProps = (dispatch: Dispatch) => ({ "sendMessage": () => dispatch({ type: "sendMessage" }) })
...
const mapStateToProps = (state: ChatDetailState) => ({
...
function myReducer(state: ChatDetailState = { msgList: [] }, action: Action) {
  switch (action.type) {
    case "sendMessage":
      console.log("fFFFF" + JSON.stringify(state))
      state.msgList.push("ccc")
      let newState = [...state.msgList]//Object.assign({ msgList: ["xx"] }, state)
